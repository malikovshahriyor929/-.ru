  ...
  return (
    <>
      <div className="flex flex-col justify-between h-full min-h-[400px]    gap-4 relative ">
        <div onClick={()=>navigate(`/productdetails/${id}`)} className="absolute top-0 left-0     rounded-lg ">
          {active == "xit" ? (
        ...
        <div onClick={()=>navigate(`/productdetails/${id}`)} className=" p-4 min-h-[200px] mt-7 flex justify-center items-center ">
          <img className="h-[150px] object-contain" src={imageUrl} alt="" />
        </div>
        <div onClick={()=>navigate(`/productdetails/${id}`)} className="flex flex-col gap-2">
          <p className="text-[13px] text-[#a8adb2]">Артикул: {article}</p>
          <h2 className=" text-[18px] font-medium ellipsis line-clamp-2 max-[440px]:!w-[350px] max-[375px]:!w-[300px] max-[320px]:!w-[250px] max-[440px]:!line-clamp-1  truncate ">
            {name}
          </h2>
